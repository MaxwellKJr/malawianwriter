        ...
        <div className='flex'>
          ...
          <ul className={toggleNav ? `absolute flex flex-col lg:hidden top-[10vh] left-0 min-h-[90vh] w-full mx-auto p-4 bg-white dark:bg-gray-900 z-50 transition-all ease-in-out duration-500 transform translate-x-[0]`
            ...
            <div className='flex-1'>
              ...
              <h5 className='text-lg font-bold mt-8 text-zinc-900 dark:text-white'>Contact Me</h5>
              <SocialLinks />
            </div>
            <Footer />
          </ul>
          ...
          <div className='flex items-center h-full'>
            <div className='lg:hidden mr-2 mt-1 sm:mt-2 p-2 hover:cursor-pointer hover:opacity-75 select-none' onClick={() => setToggleNav(!toggleNav)}>
              <FontAwesomeIcon icon={toggleNav ? faClose : faBarsStaggered} className={toggleNav ? `w-4 h-4 sm:w-5 sm:h-5 transition-all ease-in-out duration-300 text-brand` : `w-4 h-4 sm:w-5 sm:h-5 transition-all ease-in-out duration-300`} />
            </div>
            {renderThemeChanger()}
          </div>
        </div>
